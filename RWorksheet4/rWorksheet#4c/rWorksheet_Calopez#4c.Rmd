...
geom_bar(stat = "identity", fill = "purple") + theme(plot.title = element_text(hjust = 0.5),
...
```
5a.
The scatter plot displays the correlation between cylinder number and engine displacement, indicating that as cylinders increase, so does the displacement, indicating that larger engines with more cylinders tend to have greater displacement.
...
open_alexa_data <- read_excel("/cloud/project/RWorksheet4/rWorksheet#4c/alexa_file.xlsx")
